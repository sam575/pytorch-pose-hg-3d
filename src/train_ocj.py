...
def step(split, epoch, opt, dataLoader, model, criterion, optimizer = None):
  ...
  for i, (input, target2D, target3D, meta, ocv_gt, multi_cam_ind, info) in enumerate(dataLoader):
    ...
    Bar.suffix = '{split} Epoch: [{0}][{1}/{2}]| Total: {total:} | ETA: {eta:} | Loss_ocv {Loss_ocv.avg:.6f} ({Loss_ocv.val:.6f})| Acc_ocv {Acc_ocv.avg:.6f} ({Acc_ocv.val:.6f})'.format(epoch, i, nIters, total=bar.elapsed_td, eta=bar.eta_td, split = split, Acc_ocv = Acc_ocv, Loss_ocv = Loss_ocv)
  ...
  return 0,0,0,0, Acc_ocv.avg, Loss_ocv.avg
